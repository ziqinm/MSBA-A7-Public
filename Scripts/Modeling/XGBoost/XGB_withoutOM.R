...
### Get started ###
# 1. Train a model setting on TRAIN_34, then extract `ncat`, `ee`, `prediction` to a data frame;
train_34 <- read_csv("selected_34_tr.csv")
dim(train_34)

#######xgb1 MODEL######### 1 fold & 1 round
train_df=train_34 %>% 
  select(-oldmodel)
...
model2_test<-test2%>%
  select(ncat,ee)
model2_test$pred=preds2


###
test_r = model2_test
colnames(test_df) = c("ncat", "ee", "pred")
...
ggplot(plot_df) +
  ...
  labs(title = "XGBoost - TRAIN/TEST Comparation",
       subtitle = "All Variables, Without Oldmodel",
       x = "",
       y = "Lorenz") +
  theme_minimal()
...
### TRAIN_Full ###
train_full<- read_csv("Train_full_34.csv")
dim(train_full)
...
preds2_train_full = predict(xgb1, train_full_df)


model2_tr_full<-train_full%>%
  select(ncat,ee)
model2_tr_full$pred=preds2_train_full


train_f = model2_tr_full
...
### TEST_Full ###
test_full<- read_csv("Test_full_34.csv")
dim(test_full)

test_full_df =test_full %>% 
  select(-oldmodel)
...
preds2_test_full = predict(xgb1, test_full_df)


model2_te_full<-test_full%>%
  select(ncat,ee)
model2_te_full$pred=preds2_test_full


test_f = model2_te_full
...
vali_df = as.matrix(vali_df)
preds2_v = predict(xgb1, vali_df)
