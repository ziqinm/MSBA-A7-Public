...
glm1 = glm(ncat_annual ~ ., data = model_df)
summary(glm1)
pred_trainR = predict(glm1)

# Save ncat, ee, pred as a dataframe for AUC
glm1_trainR = data.frame(ncat = trainR$ncat,
                        ee = trainR$ee,
                        pred = pred_trainR)
...
pred_testR = predict(glm1, newdata = testR)

glm1_testR = data.frame(ncat = test_reduced$ncat,
                       ee = test_reduced$ee,
                       pred = pred_testR)
...
ggplot(plot_df) +
  ...
  labs(title = "Train/Test Lorenz Curve",
       subtitle = "GLM, 34 variables, default distribution, without OM",
       x = "",
       y = "Lorenz") +
  theme_minimal()
...
pred_trainF = predict(glm1, newdata = trainF)

glm1_trainF = data.frame(ncat = train_full$ncat,
                          ee = train_full$ee,
                          pred = pred_trainF)
...
pred_testF = predict(glm1, newdata = testF)

glm1_testF = data.frame(ncat = test_full$ncat,
                         ee = test_full$ee,
                         pred = pred_testF)
...
pred_valid = predict(glm1, newdata = valid)

glm1_valid = data.frame(ncat = valida$ncat,
                        ee = valida$ee,
                        pred = pred_valid)
