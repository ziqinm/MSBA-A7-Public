...
# Drop `oldmodel`
trainR = train_reduced %>% select(-oldmodel)
...
glm3 = glm(ncat_annual ~ ., data = model_df, family=poisson())
summary(glm3)
pred_trainR = predict(glm3)

# Save ncat, ee, pred as a dataframe for AUC
glm3_trainR = data.frame(ncat = trainR$ncat,
                        ee = trainR$ee,
                        pred = pred_trainR)
...
testR = test_reduced %>% select(-oldmodel)

testR = testR %>% mutate(ncat_annual = ncat/ee) %>% 
  # then remove `ee` and `ncat`
  select(-ee, -ncat)

pred_testR = predict(glm3, newdata = testR)

glm3_testR = data.frame(ncat = test_reduced$ncat,
                       ee = test_reduced$ee,
                       pred = pred_testR)
...
ggplot(plot_df) +
  ...
  labs(title = "Train/Test Lorenz Curve",
       subtitle = "GLM, 34 variables, Poisson distribution, without OM",
       x = "",
       y = "Lorenz") +
  theme_minimal()
...
trainF = train_full %>% select(-oldmodel)

trainF = trainF %>% mutate(ncat_annual = ncat/ee) %>% 
  # then remove `ee` and `ncat`
  select(-ee, -ncat)

pred_trainF = predict(glm3, newdata = trainF)

glm3_trainF = data.frame(ncat = train_full$ncat,
                          ee = train_full$ee,
                          pred = pred_trainF)
...
testF = test_full %>% select(-oldmodel)

testF = testF %>% mutate(ncat_annual = ncat/ee) %>% 
  # then remove `ee` and `ncat`
  select(-ee, -ncat)

pred_testF = predict(glm3, newdata = testF)

glm3_testF = data.frame(ncat = test_full$ncat,
                         ee = test_full$ee,
                         pred = pred_testF)
...
valid = valida %>% select(-oldmodel)

valid = valid %>% mutate(ncat_annual = ncat/ee) %>% 
  # then remove `ee` and `ncat`
  select(-ee, -ncat)

pred_valid = predict(glm3, newdata = valid)

glm3_valid = data.frame(ncat = valida$ncat,
                        ee = valida$ee,
                        pred = pred_valid)
