...
glm2 = glm(ncat_annual ~ ., data = model_df)
summary(glm2)
pred_trainR = predict(glm2)

# Save ncat, ee, pred as a dataframe for AUC
glm2_trainR = data.frame(ncat = trainR$ncat,
                        ee = trainR$ee,
                        pred = pred_trainR)
...
pred_testR = predict(glm2, newdata = testR)

glm2_testR = data.frame(ncat = test_reduced$ncat,
                       ee = test_reduced$ee,
                       pred = pred_testR)
...
ggplot(plot_df) +
  ...
  labs(title = "Train/Test Lorenz Curve",
       subtitle = "GLM, 34 variables, default distribution, with OM",
       x = "",
       y = "Lorenz") +
  theme_minimal()
...
pred_trainF = predict(glm2, newdata = trainF)

glm2_trainF = data.frame(ncat = train_full$ncat,
                          ee = train_full$ee,
                          pred = pred_trainF)
...
pred_testF = predict(glm2, newdata = testF)

glm2_testF = data.frame(ncat = test_full$ncat,
                         ee = test_full$ee,
                         pred = pred_testF)
...
pred_valid = predict(glm2, newdata = valid)

glm2_valid = data.frame(ncat = valida$ncat,
                        ee = valida$ee,
                        pred = pred_valid)
